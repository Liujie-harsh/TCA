...
from utils import evaluate_attack, cosine_similarity, cosine_similarity_per_sample
...
def untargeted_tca(model, X, Y, epsilon, alpha, iterations=200,
    ...
    for i in range(iterations):
        ...
        cos_t = cosine_similarity_per_sample(X, X_tilde)   # shape [batch]
        cos_b = cosine_similarity_per_sample(X, X_boundary)
        mask = (cos_t > cos_b).view(-1, *([1] * (X.dim() - 1)))
        X_adv = torch.where(mask, X_tilde, X_boundary)
        chosen_ratio = mask.float().mean().item() 
        ...
        if log_to_swanlab and (i % log_interval == 0 or i == iterations - 1):
            try:
                ...
                swanlab.log({
                    ...
                    f"{log_prefix}/chosen_X_tilde_ratio": chosen_ratio,
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("/home/lj/Documents/TCN/best_cnn_model.pth", map_location="cpu"))
    model.eval()
    model.to("cpu")
...
#===============epsilon修改=====================
    epsilon = 0.2
    run = swanlab.init(
        project="PGD_Attack_Experiments",
        name=f"{epsilon}",
        config={
            "model": "TimeSeriesCNN",
            "attack": "untargeted_PGD",
            "epsilon": epsilon,
            "alpha": 0.001,
            "iterations": 200,
    ...
    # ===================攻击切换=====================
    X_adv_untargeted = untargeted_PGD(model, X, Y, swanlab.config["epsilon"], swanlab.config["alpha"],
                                            log_to_swanlab=True, log_prefix="untargeted_PGD",
                                         )  
    # 评估攻击效果
